...
<mark>To do (maybe not all on the same page)</mark>

- How do you run your own AS
- How does the AS talk to the RS
- What are the introspection points between the RS and AS
...
In Open Payments, the client must [request a grant](/apis/auth-server/operations/post-request) before calling the Open Payments API to create an [incoming payment](/apis/resource-server/operations/create-incoming-payment), [outgoing payment](/apis/resource-server/operations/create-outgoing-payment), and/or [quote](/apis/resource-server/operations/create-quote). A client can request a single grant for each one, but not for multiples of each. For example, a client can request a single grant for an incoming and outgoing payment, but not for two incoming payments.
